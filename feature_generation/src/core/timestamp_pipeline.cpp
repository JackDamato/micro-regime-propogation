...
void TimestampPipeline::run(uint64_t snapshot_interval_ns,
    ...
    while (base_opt && future_opt) {
        ...
        if (base_event.timestamp_ns >= next_snapshot_time &&
            ...
            base_data_reciever.ingest_feature_set(base_asset_, base_event.timestamp_ns,
            ...
            future_data_reciever.ingest_feature_set(future_, future_event.timestamp_ns,
                           fut_raw, fut_norm_short, fut_norm_long);

            next_snapshot_time = std::min(base_event.timestamp_ns, future_event.timestamp_ns) + snapshot_interval_ns;
        }
    }
    if (base_opt) {
...
uint64_t TimestampPipeline::getNYSEStartTime(const std::string& date_str) {
    std::tm timeinfo = {};
...
uint64_t TimestampPipeline::getNYSEEndTime(const std::string& date_str) {
    std::tm timeinfo = {};
    timeinfo.tm_year = std::stoi(date_str.substr(0, 4)) - 1900;
    timeinfo.tm_mon  = std::stoi(date_str.substr(4, 2)) - 1;
    timeinfo.tm_mday = std::stoi(date_str.substr(6, 2));
