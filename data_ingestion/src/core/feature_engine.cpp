...
FeatureInputSnapshot FeatureEngine::generate_snapshot(uint64_t timestamp_ns) {
    ...
    while (!rolling_state_.trade_times.empty() && rolling_state_.trade_times.front() < timestamp_ns - microregime::EVENT_WINDOW_NS) {
    ...
    while (!rolling_state_.order_times.empty() && rolling_state_.order_times.front() < timestamp_ns - microregime::EVENT_WINDOW_NS) {
...
void FeatureEngine::UpdateRollingStats() {
    ...
    if (rolling_state_.midprices.size() > 1) {
        ...
        double log_return = std::log(current) - std::log(previous);
        rolling_state_.returns.push_back(log_return;)
    } else {
    ...
    if (rolling_state_.returns.size() > ROLLING_WINDOW) {
        rolling_state_.returns.pop_front();
    }
}
