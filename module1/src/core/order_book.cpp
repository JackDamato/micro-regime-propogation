...
double OrderBookManager::BestBidPrice() const {
    return bid_book_.empty() ? 0.0 : bid_book_.begin()->first;
}

double OrderBookManager::BestAskPrice() const {
    return ask_book_.empty() ? std::numeric_limits<double>::max() : ask_book_.begin()->first;
}

int OrderBookManager::BestBidSize() const {
    if (bid_book_.empty()) return 0;
    int total_size = 0;
    for (const auto& order : bid_book_.begin()->second) {
        total_size += order.size;
    }
    return total_size;
}

int OrderBookManager::BestAskSize() const {
    if (ask_book_.empty()) return 0;
    int total_size = 0;
    for (const auto& order : ask_book_.begin()->second) {
        total_size += order.size;
    }
    return total_size;
}
